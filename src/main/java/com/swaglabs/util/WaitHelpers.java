...
public class WaitHelpers {
    ...
    public static void toBeClickable(WebElement element) {
        wait = new WebDriverWait(BaseDriver.getDriver(), waitTime);
    ...
    public static boolean waitForJQuaryToLoad() {
        ...
        ExpectedCondition<Boolean> jQueryLoad = new ExpectedCondition<Boolean>() {
            ...
            public Boolean apply(WebDriver driver) {
                try {
                    return ((Long) executeJavaScript(BaseDriver.getDriver(), "return jQuery.active") == 0);
                } catch (Exception e) {
                    return true;
                }
            }
    ...
    public static boolean waitForJSToLoad() {
        wait = new WebDriverWait(BaseDriver.getDriver(), 30);
        // wait for Javascript to load
        ExpectedCondition<Boolean> jsLoad = new ExpectedCondition<Boolean>() {

            @Override
            public Boolean apply(WebDriver driver) {
                return executeJavaScript(BaseDriver.getDriver(), "return document.readyState")
